...
import { MARKETPLACES, MARKETPLACE_SLUGS, money, type Marketplace } from "@/data/demo";
import {
  buyerPrice,
  shippingFor,
  shippingIncluded,
...
function FakeListing() {
  ...
  const page =
    name === "eBay" ? (
      <EbayPage listing={listing} />
    ) : name === "Etsy" ? (
      <EtsyPage listing={listing} />
    ) : name === "Square" ? (
      <SquarePage listing={listing} />
    ) : (
      <FacebookPage listing={listing} />
    );
